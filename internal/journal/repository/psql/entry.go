...
func (r *psqlJournalEntryRepository) Create(transactionID string, journalAccountID string, amount money.Money) error {
	query := `
		INSERT INTO
			journal_entry(
				transaction_id,
	...
	_, err = stmt.Exec(transactionID, journalAccountID, money.ToCents(amount))
	if err != nil {
		return err
	}
	return nil
